...
if __name__ == "__main__":
    ...
    loss_df = pd.DataFrame(columns=lr_list)
    acc_df = pd.DataFrame(columns=lr_list)
    pred_y_acc = []
    for lr in lr_list:
        loss_history, acc_history, train_theta, train_bias = (
            train(X_train, y_train, theta, bias, lr, epochs))
        loss_df[lr] = loss_history
        acc_df[lr] = acc_history
    ...
    plt.show()
    plt.close()
    for lr, pred_acc, final_loss in zip(lr_list, pred_y_acc, loss_df.iloc[-1]):
        print("lr: {}, final_loss: {}, pred_acc: {}".format(lr, final_loss, pred_acc))
